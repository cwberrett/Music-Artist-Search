...
var artObj = {
    ...
    getArtist: function() {
        let id = this.id;
        let call = $.getJSON('https://api.spotify.com/v1/artists/' + id);
        call.done(function(data) {
            artObj.person = data;
            let albCall = $.getJSON('https://api.spotify.com/v1/artists/' + id + '/albums?limit=10&offset=' + artObj.albOffset);
            albCall.done(function(data) {
                artObj.albums = data;
                let songCall = $.getJSON('https://api.spotify.com/v1/artists/' + id + '/top-tracks?country=US');
    ...
    displayArtist: function() {
        ...
        let songs = this.topSongs.tracks;
...
var albObj = {
    id: "",
    getAlbum: function() {
        let call = $.getJSON('https://api.spotify.com/v1/albums/' + this.id);
        call.done(function(data) {
            albObj.album = data;
            let artCall = $.getJSON('https://api.spotify.com/v1/artists/' + albObj.album.artists[0].id + '/albums?limit=10');
    ...
    displayAlbum: function() {
        ...
        let tracks = this.album.tracks.items;
...
var songObj = {
    ...
    offset: 0,
    getSong: function() {
        let call = $.getJSON("https://api.spotify.com/v1/tracks/" + this.id);
        call.done(function(data) {
            songObj.track = data;
            let albCall = $.getJSON(songObj.track.album.href);
            albCall.done(function(info) {
                songObj.album = info;
                let artCall = $.getJSON('https://api.spotify.com/v1/artists/' + songObj.track.artists[0].id + '/albums?limit=10&offset=' + songObj.offset);
    ...
    displaySong: function() {
        ...
        $.each(this.otherAlbs.items, function(key, value) {
            if (value.name != albComp) {
                let entry2 = "<li id='" + value.id + "'>" + value.name + "</li>";
...
var searchObj = {
    ...
    getList: function(query, type) {
        this.query = query;
        let call = $.getJSON("https://api.spotify.com/v1/search?limit=10&type=" + type + "&q=" + this.query + "&offset=" + this.offset);
        call.done(function(data) {
            searchObj.list = data;
            let term = type + 's';
            if (searchObj.list[term].items.length !== 0) {
                ...
                $("#" + type + " .results").show();
                $(".results ul").empty();
                let entry = "<li>Sorry, your search returned no results.</li>";
    ...
    displayList: function(type) {
        ...
        $(".results ul").empty();
        let listName = type + 's';
        let list = this.list[listName].items;
        $.each(list, function(key, value) {
            var entry;
            if (type == 'album') {
                let artName;
                let albTemp = $.getJSON(value.href);
